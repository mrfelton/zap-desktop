...
const Autopilot = ({ autopilot, setAutopilot }) => (
  <div className={styles.container}>
    <section className={`${styles.option} ${autopilot ? styles.active : undefined}`}>
      <div className={`${styles.button}`} onClick={() => setAutopilot(true)}>
        {autopilot ? <FaCircle /> : <FaCircleThin />}
        <span className={styles.label}>Enable Autopilot</span>
      </div>
    </section>
    <section
      className={`${styles.option} ${!autopilot && autopilot !== null ? styles.active : undefined}`}
    >
      <div className={`${styles.button}`} onClick={() => setAutopilot(false)}>
